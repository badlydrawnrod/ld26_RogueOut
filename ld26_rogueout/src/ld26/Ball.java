...
class Ball {
    ...
    public Ball(RogueOut game, Dungeon dungeon, Bat bat,
            SpriteBatch spriteBatch, BitmapFont font) {
        this.game = game;
    ...
    private boolean hit() {
        return check(x, y) || check(x + fontWidth - 1, y)
    ...
    private boolean check(float x, float y) {
        ...
        int damageCaused = isStrong && e.isMonster() ? e.hitPoints()
    ...
    private boolean hitBat() {
        ...
        if (dy < 0) {
            ...
            if (right >= bat.left() && left <= bat.right()
                    && bottom <= bat.top() && bottom - dy > bat.top()) {

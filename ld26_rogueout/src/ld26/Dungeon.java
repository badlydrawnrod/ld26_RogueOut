...
class Dungeon extends TerminalWriter {
    ...
    private BaseEntity makeGoblin() {
        return new BaseEntity(this, "gggg", "the goblin", 0).isMonster(true)
                .level(0);
    }

    private BaseEntity makeOrc() {
        return new BaseEntity(this, "oooo", "the orc", 0).isMonster(true)
                .level(1);
    }

    private BaseEntity makeAcidBlob() {
        return new BaseEntity(this, "aa", "the acid blob", 0).isMonster(true)
                .level(1);
    }

    private BaseEntity makeDwarfLord() {
        return new BaseEntity(this, "hhhh", "the dwarf lord", 0)
                .isMonster(true).level(4);
    }

    private BaseEntity makeTrap() {
        return new BaseEntity(this, "^^", "a trap", 1).setDestructible(1);
    }

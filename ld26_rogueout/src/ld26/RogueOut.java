...
public class RogueOut extends com.badlogic.gdx.Game {
	...
	private enum Mode { SplashScreen, Playing, Death };
	...
	public void create() {
        ...
        // Load the font.
        font = new BitmapFont(Gdx.files.internal("assets/data/consolas32.fnt"), false);
        ...
        // Allow the game to get mouse input, even when it's outside of the window.
        Gdx.input.setCursorCatched(true);
        
        // Load the music.
        splashScreenMusic = Gdx.audio.newMusic(Gdx.files.internal("assets/data/loop.ogg"));
        gameMusic = Gdx.audio.newMusic(Gdx.files.internal("assets/data/gameLoop.ogg"));
	...
	private Sound loadSound(String filename) {
		return Gdx.audio.newSound(Gdx.files.internal("assets/data/" + filename + ".ogg"));
	}
	...
	private void updatePlaying(float delta) {
		...
		if (wasBallAlive) {
			...
			float speedMultiplier = (stateTime >= speedTime)
					? 1.0f
					: 2.0f;
			float slowMultiplier = (stateTime >= slowTime)
					? 1.0f
					: 0.5f;
	...
	private float updateMouse() {
		float multiplier = (stateTime >= confusedTime)
				? 1
				: -1;

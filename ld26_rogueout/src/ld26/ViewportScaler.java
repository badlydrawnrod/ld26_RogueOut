...
public class ViewportScaler {
    ...
    public Vector2 scale(float w, float h) {
        float aspect = w / h;
        if (aspect <= maxAspect) {
            // Keep h constant, changing w. If aspect < minAspect then this
            // results in letterboxing at the top and
            // bottom of the viewport.
            result.set(this.h * Math.max(aspect, minAspect), this.h);
        }
        else {
            // Keep w constant, changing h. This results in letterboxing at the
